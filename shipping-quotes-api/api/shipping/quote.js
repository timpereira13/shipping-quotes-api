...
module.exports = async (req, res) => {
  ...
  // --- DEMO SHORT-CIRCUIT ---
  if (FORCE_DEMO || (req.query && req.query.demo === '1')) {
    return res.status(200).json(demoResponse());
  }
...
/* ----------- Live carrier helpers (unchanged; only used when FORCE_DEMO=false) ----------- */
async function getUpsRates(input) {
  ...
  const r = await fetch('https://onlinetools.ups.com/api/rating/v2403/Rate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${await upsAuth()}` },
