...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!quotes.length) return res.status(502).json({ error: 'No rates from carriers' });

    const filters = body.service_filters || [];
    const filtered = quotes.filter(q => filterServices(q, filters))
...
function filterServices(q:any, filters:string[]){
...
// UPS
async function getUpsRates(input:any){
  ...
  const payload = {
    RateRequest: {
      ...
      Shipment: {
        ...
        ShipTo: { Address: { PostalCode: String(input.dest_zip), CountryCode:'US', ResidentialAddressIndicator: input.residential ? '' : undefined } },
        Package: [{
          ...
          PackageServiceOptions: input.declared_value ? { DeclaredValue: { CurrencyCode:'USD', MonetaryValue:String(input.declared_value) } } : undefined
        }],
        ShipmentRatingOptions: { RateChartIndicator: '' },
        DeliveryTimeInformation: input.ship_date ? { PackageBillType: '03', Pickup: { Date: input.ship_date.replaceAll('-','') } } : undefined
      }
    }
  };
...
// FedEx
async function getFedexRates(input:any){
  ...
  return details.flatMap((d:any) => {
    ...
    const amounts = d?.ratedShipmentDetails?.[0]?.totalNetCharge || d?.ratedShipmentDetails?.[0]?.shipmentRateDetail?.totalNetChargeWithDutiesAndTaxes;
...
function parseTransit(s:string){
  const m = /(\w+)_DAYS?/.exec(s||'');
  if(!m) return undefined;
  const map:any = {ONE:1,TWO:2,THREE:3,FOUR:4,FIVE:5,SIX:6,SEVEN:7};
